    ...
    with torch.no_grad():
        ...
        for i in range(patient_data.shape[0]):
            t, old_shape = pad_patient_3D(
                patient_data[i], new_shape_must_be_divisible_by, min_size
            )
        ...
        for i in range(num_repeats):
            for m in range(x):
                ...
                if (
                    ...
                    and (3 in mirror_axes)
                    and (4 in mirror_axes)
                ):
                    do_stuff = True
                ...
                if do_stuff:
                    ...
                    p = net(
                        a
                    )  # np.copy is necessary because ::-1 creates just a view i think
                    p = p.data.cpu().numpy()
